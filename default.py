...
def list_videos():
    ...
    for item in items:
        ...
        liz = xbmcgui.ListItem( a.find('h2').text)
        liz.setArt({'icon': '', 'thumb': base_url + a.find('img').attrs['src']})
        liz.setInfo(type="Video", infoLabels={"Title": a.find('h2').text})
...
def playvideo(video_id,  client_location, source_id=None):
    ...
    url_api_url = 'http://vas.sim-technik.de/vas/live/v2/videos/%s/sources/url?%s' % (video_id, urlencode({
...
def geturl(url, data=None, header=None):
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:49.0) Gecko/20100101 Firefox/49.0'} if not header else header
    try:
        response = requests.get(url, params=data, timeout=20, headers=headers)
